...
class CBAM_ASPP(nn.Module):                       ##加入通道注意力机制
    def __init__(self, dim_in, dim_out=64, rate=1, bn_mom=0.1,):
...
class Conv_SE(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.act = GELU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.se =simam_module(c2)

    def forward(self, x):
        # print("conv的大小",x.size())
...
class SPPCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        # cbamaspp = self.cbam(torch.cat((y1, y2), dim=1))
...
class RepConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, act=True, deploy=False):
        ...
        if deploy:
            ...
            # self.rbr_reparam = DeformConv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=True)
